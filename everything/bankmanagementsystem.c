...
void main()
{
        char username[10], password[20], us[20], pas[20];
        int opt;
        system("Color F0");
        /*for login into the system*/
        FILE*pass;
        pass=fopen("bin.dat", "r");
        login:
        system("cls");
        ...
        printf("\t\t\t\t\tPASSWORD:");
        scanf("%s", password);
        while(fscanf(pass, "%s %s", us, pas)!=EOF)
        {
            if((strcmp(username,us)==0&&strcmp(password,pas)==0))
            {
                test++;
                printf("\n\nLOGIN SUCCESSFUL!\nLOADING");
                for(i=0;i<6;i++)
        ...
        else
        {
            ...
            printf("\n\tDo you want to retry or exit?(Enter 1 to retry and any other number to exit):");
            scanf("%d", &opt);
            if(opt==1)
                {
                    test=0;
...
void new_account()
{
    ...
    system("cls");
	accno_same:
    check_acc_no=rand()%20;
    while(fread(&test_acc, sizeof(test_acc), 1, add))
    {
        if(test_acc.acc_no==check_acc_no)
            goto accno_same;
    }
    printf("\n\n\nacc no: %d", check_acc_no);
...
void update()
{
    ...
    while(fread(&up,sizeof(up), 1, real))
    {
        ...
        {
            test++;
            printf("\t\t\t\tAccount found!\nName:%s %s\nAddress:%s\nDOB:%d/%d/%d\nAccount No.:%d\nCitizenship no:%s\nContact number:%.0lf\n", up.first_name, up.last_name, up.address, up.dob.y, up.dob.m, up.dob.d, up.acc_no, up.citizenship, up.phone);
...
void view_list()
{
        ...
        {
            printf("\t\t\t\t\t%d\t%s %s\n", viewloan.loan_id, viewloan.first_name, viewloan.last_name);
        }
        again:
...
void loan()
{
    ...
    else if(i==2)
    {
        ...
        while(fread(&a, sizeof(a), 1, londet))
        {
            ...
            {
                test++;
                printf("\t\t\tLoan id:%d\n\t\t\tName:%s %s\n\t\t\tAddress:%s\n\t\t\tPhone number:%.0lf\n\t\t\tLoan amount:%f\n\t\t\tLoan deposit:%s", a.loan_id, a.first_name, a.last_name, a.address, a.phone, a.loan_amt, a.loan_deposit);
         ...
         while(fread(&transact_acc, sizeof(transact_acc), 1, fptr))
        {
            if(transact_acc.acc_no==acc)
            {
                ...
                else
                {
                    ...
                    if(transact_acc.amt<(withdraw_amt+500))
                       {
                           printf("\n\t\t\tInsufficient Balance!!|Can not withdrawal\n");
                       }
                    else
                        {
